    ...
    const InstructionTable& instruction_table, ExecutionState& state) noexcept
{
    ...
    if constexpr (traits.stack_height_change > 0)
    {
        if (INTX_UNLIKELY(stack_size == Stack::limit))
            return EVMC_STACK_OVERFLOW;
    }

    if constexpr (traits.stack_height_required > 0)
    {
        if (INTX_UNLIKELY(stack_size < traits.stack_height_required))
...
using InstrFn1 = void(ExecutionState&) noexcept;
static_assert(std::is_same_v<decltype(add), InstrFn1>);

/// The signature of basic instructions which may fail.
using InstrFn2 = evmc_status_code(ExecutionState&) noexcept;
static_assert(std::is_same_v<decltype(exp), InstrFn2>);

/// The signature of terminating instructions.
using InstrFn3 = StopToken(ExecutionState&) noexcept;
static_assert(std::is_same_v<decltype(stop), InstrFn3>);

/// The signature of instructions requiring access to current code position.
using InstrFn4 = code_iterator(ExecutionState&, code_iterator) noexcept;
static_assert(std::is_same_v<decltype(push<1>), InstrFn4>);
static_assert(std::is_same_v<decltype(pc), InstrFn4>);
static_assert(std::is_same_v<decltype(jump), InstrFn4>);
...
template <>
[[gnu::always_inline]] inline code_iterator invoke<InstrFn1*>(
    InstrFn1* instr_fn, ExecutionState& state, code_iterator pos) noexcept
{
    instr_fn(state);
    return pos + 1;
}
...
template <>
[[gnu::always_inline]] inline code_iterator invoke<InstrFn2*>(
    InstrFn2* instr_fn, ExecutionState& state, code_iterator pos) noexcept
{
...
template <>
[[gnu::always_inline]] inline code_iterator invoke<InstrFn3*>(
    InstrFn3* instr_fn, ExecutionState& state, code_iterator /*pos*/) noexcept
{
...
template <>
[[gnu::always_inline]] inline code_iterator invoke<InstrFn4*>(
    InstrFn4* instr_fn, ExecutionState& state, code_iterator pos) noexcept
{
